...
save(beers_and_breweries,file="beers_and_breweries.Rdata")
```
...
breweries <- breweries %>% dplyr::arrange(desc(overall_brewery_averge)) %>% dplyr::slice(1:5)
```
...
pdf(file=paste("../figures/",brewery_string, "_beers.pdf", sep=""),width=16, height = 9)
...
breweries <- breweries %>% arrange(overall_brewery_averge) %>% slice(1:5)
```
